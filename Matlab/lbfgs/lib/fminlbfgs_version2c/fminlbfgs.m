...
if(strcmp(optim.Display,'iter'))
    disp('     Iteration  Func-count   Grad-count         f(x)         x(1)         x(2)');
end
...
if(strcmp(optim.Display,'iter'))
        s=sprintf('     %5.0f       %5.0f       %5.0f       %13.6g       %5.2f       %5.2f',...
            data.iteration,data.funcCount,data.gradCount,data.fInitial,data.xInitial(1),data.xInitial(2)); disp(s);
end
...
%     end
%% the above linesearch was disabled, we need to fix a few things first
[data,f_alpha, grad]=gradient_function(data.xInitial(:)+1*data.dir(:),funfcn, data, optim);
fPrime_alpha = grad'*data.dir(:);

% Store the found alpha values
data.alpha=1; % don't use linesearch
